...
const RAW_CITY_GEO_PATH = path.resolve(__dirname, 'src/assets/geo/hunan-cities.raw.geo.json')
...
const getCityAdcode = (adcode, parentAdcode) => {
  if (parentAdcode) {
    return String(parentAdcode)
  }

  const normalizedAdcode = String(adcode || '').padStart(6, '0')
  return /^\d{6}$/.test(normalizedAdcode) ? `${normalizedAdcode.slice(0, 4)}00` : ''
}
  ...
  countyGeoJson.features.map((feature) => {
    ...
    return {
      ...
      properties: {
        ...
        adcode: String(properties.adcode || ''),
        cityAdcode: getCityAdcode(properties.adcode, properties.parent?.adcode),
        center: normalizePoint(properties.center),
        centroid: normalizePoint(properties.centroid || properties.center),
        level: 'county'
      },
...
const buildCityMapFeatures = (cityGeoJson) => (
  cityGeoJson.features.map((feature) => {
    const properties = feature?.properties || {}
...
const createHunanCountyDataset = () => {
  ...
  const mapOutput = {
    ...
    features: [...countyFeatures, ...cityFeatures, provinceOutlineFeature]
  }
  ...
  console.log(`Counties: ${counties.length}, Cities: ${cityFeatures.length}, Years: ${YEARS[0]}-${YEARS[YEARS.length - 1]}`)
